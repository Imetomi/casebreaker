...
def get_case_count(db: Session, subtopic_id: int) -> int:
    ...
    return db.query(func.count(CaseStudyModel.id)).filter(
        CaseStudyModel.subtopic_id == subtopic_id
    ).scalar()
...
def get_case_study_by_id_or_404(db: Session, case_study_id: int) -> CaseStudyModel:
    ...
    case_study = db.query(CaseStudyModel).filter(CaseStudyModel.id == case_study_id).first()
