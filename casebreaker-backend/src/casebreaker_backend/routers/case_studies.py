...
def create_case_study(case_study: CaseStudyCreate, db: Session = Depends(get_db)):
    ...
    subtopic = db.query(SubtopicModel).filter(SubtopicModel.id == case_study.subtopic_id).first()
    if not subtopic:
...
def list_case_studies(
    ...
    if difficulty:
        query = query.filter(CaseStudyModel.difficulty == difficulty)
    return query.all()

@router.get("/{case_study_id}", response_model=CaseStudy)
def get_case_study(case_study_id: int, db: Session = Depends(get_db)):
    db_case_study = db.query(CaseStudyModel).filter(CaseStudyModel.id == case_study_id).first()
...
def get_case_study_by_slug(share_slug: str, db: Session = Depends(get_db)):
    db_case_study = db.query(CaseStudyModel).filter(CaseStudyModel.share_slug == share_slug).first()
    if db_case_study is None:
        raise HTTPException(status_code=404, detail="Case study not found")
    return db_case_study
